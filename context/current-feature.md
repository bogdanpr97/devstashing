# Current Feature

## Status

In Progress

## Goals

Dashboard Phase 3 — main content area.

- 4 stats cards at the top (total items, collections, favorite items, favorite collections)
- Recent collections section
- Pinned items section
- 10 recent items section

## Notes

- Use mock data from `src/lib/mock-data.ts`
- Reference: `context/screenshots/dashboard-ui-main.png`
- Spec: `context/features/dashboard-phase-3-spec.md`

## History

<!-- Keep this updated. Earliest to latest -->

- **2026-03-18** — Initial Next.js project setup. Removed default boilerplate (placeholder SVGs, default page, globals.css). Added CLAUDE.md and context files. Pushed to GitHub.
- **2026-03-18** — Dashboard Phase 1: Initialized ShadCN UI (v4, Tailwind v4 compatible). Installed Button and Input components. Added `/dashboard` route with top bar (search, New Collection, New Item buttons), sidebar placeholder, and main area placeholder. Set dark mode as default via `<html class="dark">`. Added type color CSS variables to globals.css.
- **2026-03-20** — Dashboard Phase 2: Collapsible sidebar with open/close toggle, item type links to `/items/TYPE`, favorite collections section, most recent collections section, user avatar area at the bottom, mobile drawer behavior. Uses mock data from `src/lib/mock-data.ts`.